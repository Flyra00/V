...
var tests = new (string Name, Func<Task> Run)[]
{
    ...
    ("Cash payment records received and change", PaymentManagementTests.ConfirmPaymentAsync_Cash_SetsAmountReceivedAndChange),
    ("Cash payment rejects insufficient amount", PaymentManagementTests.ConfirmPaymentAsync_Cash_RejectsInsufficientAmount),
    ("Cashier promo apply ignores minimum purchase", PaymentManagementTests.ApplyPromoAsync_AllowsManualApply_IgnoresMinimumPurchase),
    ("Cashier promo apply fails on paid transaction", PaymentManagementTests.ApplyPromoAsync_FailsForPaidTransaction),
    ("Cashier promo remove resets transaction totals", PaymentManagementTests.RemovePromoAsync_SucceedsWhenPending),
    ("Cash payment after promo uses updated total", PaymentManagementTests.ConfirmPaymentAsync_AfterPromo_UsesUpdatedTotal),
    ("Kasir receipt is paid-only", PaymentManagementTests.PrintReceipt_PaidOnlyGuard_Works),
    ("Online payment cannot be manually confirmed", PaymentManagementTests.ConfirmPaymentAsync_RejectsOnlineManualConfirmation),
    ("Midtrans settlement marks payment paid", PaymentManagementTests.CheckMidtransStatusAsync_Settlement_MarksPaid),
    ("Midtrans pending keeps payment pending", PaymentManagementTests.CheckMidtransStatusAsync_Pending_KeepsPending),
    ("Midtrans missing transaction keeps payment pending", PaymentManagementTests.CheckMidtransStatusAsync_NotFound_KeepsPending),
    ("Midtrans expire marks payment cancelled", PaymentManagementTests.MidtransService_ExpireStatus_MarksPaymentCancelled),
    ("Midtrans webhook valid signature marks paid", PaymentManagementTests.MidtransService_ProcessNotificationAsync_ValidSignature_MarksPaid),
    ("Midtrans webhook invalid signature is rejected", PaymentManagementTests.MidtransService_ProcessNotificationAsync_InvalidSignature_IsRejected),
    ("Midtrans webhook mismatched gross amount is rejected", PaymentManagementTests.MidtransService_ProcessNotificationAsync_MismatchedGrossAmount_IsRejected),
    ("Midtrans stale pending does not downgrade paid payment", PaymentManagementTests.MidtransService_StalePendingStatus_DoesNotDowngradePaidPayment),
    ("Midtrans create snap exposes provider error", PaymentManagementTests.MidtransService_CreateSnapTransactionAsync_IncludesProviderErrorMessage),
    ...
    ("Table deactivate cancels active session and disabled table blocks session", TableSessionTests.TableService_DeactivateAsync_CancelsActiveSession_AndDisabledSessionCreation),
